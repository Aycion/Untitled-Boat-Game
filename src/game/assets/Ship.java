...
public class Ship extends GameObject implements Moveable, Collidable {
    ...
    public Ship(EngineCore engine, AffineTransform transform) throws ResourceNotFound {
        ...
        this.spriteImage = this.shipSprite;

        this.attachComponent(this.spriteImage);

        // Calculate the rotation anchor points using the
        //  size of the sprite
        this.rotAnchorX = this.shipRotAnchorX = this.spriteImage.getWidth() / 2.0;
        this.rotAnchorY = this.shipRotAnchorY = this.spriteImage.getHeight() / 2.0;
        ...
        this.attachComponent(this.activeCollider);
    ...
    protected void replaceCollider(Collider replacement) {
        super.detachComponent(this.activeCollider);

        this.activeCollider = replacement;

        super.attachComponent(this.activeCollider);
    }

    protected void replaceSprite(SpriteImage replacement) {
    ...
    public void logic() {
        ...
        if (InputCaptor.bindingActive("F")) {
            if (super.getCompoment(this.shipSprite) != null) {
                this.lpTimer++;
                this.shipTimer = 0;
            } else {
                this.shipTimer++;
            ...
            if (this.lpTimer > 20) {
                super.detachComponent(this.shipSprite);
                super.attachComponent(this.lpSprite);
                ...
                super.detachComponent(this.lpSprite);
                super.attachComponent(this.shipSprite);
